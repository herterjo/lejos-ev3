...
public class EV3I2CPort extends EV3IOPort implements I2CPort {
     ...
     * @param writeLen      The number of bytes to write.
     */
    public synchronized void i2cWrite(int deviceAddress, byte[] writeBuf, int writeOffset, int writeLen) {
        i2cGeneral(deviceAddress, writeBuf, writeOffset, writeLen, 0);
    }

    private synchronized void i2cGeneral(int deviceAddress, byte[] writeBuf,
                                         int writeOffset, int writeLen, int readLen) {
        cmd[0] = (byte) port;
        cmd[1] = speed;
        cmd[2] = (byte) readLen;
        cmd[3] = (byte) writeLen;
